...
import { listTickets as apiList, createTicket as apiCreate, updateTicket as apiUpdate, deleteTicket as apiDelete, } from "./lib/api";
export default function App() {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                const normalized = rows.map((r) => withComputed({
                    id: r.ticket_id ?? r.id,
                ...
                console.error(e);
                if (isAlive)
                    setLoadError("Failed to load records from server.");
            }
            finally {
                if (isAlive)
                    setLoading(false);
    ...
    const updateTicket = async (id, patch) => {
        ...
        try {
            ...
            if ("legs" in payload)
                payload.legs = normalizeLegs(payload.legs);
    ...
    return (_jsx(ErrorBoundary, { children: _jsxs("div", { className: "shell", children: [_jsx(Topbar, {}), _jsx("div", { className: "card", style: { marginBottom: 12 }, children: _jsxs("div", { className: "grid3", children: [_jsxs("div", { children: [_jsx("label", { children: "Range (start)" }), _jsx("input", { type: "date", value: range.start, onChange: (e) => setRange((r) => ({ ...r, start: e.target.value })) })] }), _jsxs("div", { children: [_jsx("label", { children: "Range (end)" }), _jsx("input", { type: "date", value: range.end, onChange: (e) => setRange((r) => ({ ...r, end: e.target.value })) })] }), _jsxs("div", { style: { display: "flex", alignItems: "flex-end", gap: 8 }, children: [loading ? _jsx("span", { className: "pill soft", children: "Loading\u2026" }) : _jsx("span", { className: "pill good", children: "Synced" }), loadError && _jsx("span", { className: "pill bad", children: loadError })] })] }) }), _jsx(Tabs, { tickets: tickets, addTicket: addTicket, updateTicket: updateTicket, setTickets: setTickets, globalDomains: globalDomains })] }) }));
}
